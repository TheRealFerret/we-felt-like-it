...
class PlayState extends MusicBeatState
{
	...
	//i felt like it okay jeez
	var hall:FlxBackdrop;
	var lights:FlxBackdrop;
	var pillar:FlxBackdrop;
	var markBody:BGSprite;
	var startRunning:BGSprite;
	var rfylMechInfo:BGSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'hallway':
				...
				hall.y -= 120;
				hall.velocity.set(-4000, 0);
				add(hall);

				lights = new FlxBackdrop(Paths.image('hallway/lightpass', 'yeah'), 0, 0);
				lights.y -= 125;
				lights.velocity.set(-4000, 0);
				lights.scale.set(2, 2);

				pillar = new FlxBackdrop(Paths.image('hallway/pillar', 'yeah'), 0, 0);
				pillar.y -= 200;
				pillar.velocity.set(-4000, 0);

				markBody = new BGSprite('hallway/markbody', 2270, 610, ['running body'], true);

				startRunning = new BGSprite('hallway/start_running', ['start runnin mate'], false);
				startRunning.animation.addByPrefix('start', 'start runnin mate', 24, false);
				startRunning.screenCenter();
				startRunning.cameras = [camOther];
				add(startRunning);
				startRunning.alpha = 0;

				rfylMechInfo = new BGSprite('hallway/im_sorry_for_torturing_you_tormented', ['open text'], false);
				rfylMechInfo.animation.addByPrefix('text', 'funny', 24, false);
	...
	function alternateAttack()
	{
		...
		new FlxTimer().start(1.51, function(tmr:FlxTimer){
			if (markShootingHit)
				markShootingHit = false;
		});
	}
	...
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			...
			psychDialogue.cameras = [camNotes];
			add(psychDialogue);
		} else {
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player == 1)
			{
				...
				if (!ClientPrefs.middleScroll && SONG.song.toLowerCase() == 'captive')
					{
						babyArrow.x += -300;
					}
			}
	...
	override public function update(elapsed:Float)
	{
			...
			case 'hallway':
				if (curStep > 127){
					if (FlxG.keys.justPressed.SPACE){
						FlxG.sound.play(Paths.sound('gunshotmark'), 0.5);
						boyfriend.playAnim('shooting', false);
						boyfriend.specialAnim = true;
		...
		if(curStage == 'hallway'){
			if (boyfriend.animation.curAnim.name.startsWith('idle') || boyfriend.animation.curAnim.name.startsWith('shooting'))
				markBody.alpha = 0;
			else
				markBody.alpha = 1;
		}
		...
		else
		{
			if (!paused)
			{
				...
				if(updateTime) {
					...
					songPercent = (curTime / timeBarLength);

					var songCalc:Float = (timeBarLength - curTime);
		...
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				if (SONG.song.toLowerCase() == 'captive')
					WindowsNotification.sendFakeMsgBox(captiveDeathMessages[new FlxRandom().int(0, captiveDeathMessages.length - 1)]);
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Cinematic Bars':
				var timeVal:Float = Std.parseFloat(value1);
				var heightVal:Float = Std.parseFloat(value2);
				...
				if(Math.isNaN(heightVal)) heightVal = 200;
				...
				FlxTween.tween(upBar, {y: (FlxG.height - upBar.height) / 2 - heightVal}, (Conductor.crochet / 1000) / 2, {ease: FlxEase.expoInOut, onComplete: function(tween:FlxTween)
				{
					new FlxTimer().start(timeVal, function(timer:FlxTimer)
					{
				...
				FlxTween.tween(downBar, {y: (FlxG.height - downBar.height) / 2 + heightVal}, (Conductor.crochet / 1000) / 2, {ease: FlxEase.expoInOut, onComplete: function(tween:FlxTween)
				{
					new FlxTimer().start(timeVal, function(timer:FlxTimer)
					{
			...
			case 'Set Default Cam Zoom':
				var dczVal:Float = Std.parseFloat(value1);
				if(Math.isNaN(dczVal)) defaultCamZoom = 0.9;
				defaultCamZoom = dczVal;
			case 'Screen Flash':
				var colorVal:FlxColor;
				switch(value1.toLowerCase().trim()) {
					case 'white':
						colorVal = FlxColor.WHITE;
					case 'black':
						colorVal = FlxColor.BLACK;
					case 'green':
						colorVal = FlxColor.GREEN;
					case 'blue':
						colorVal = FlxColor.BLUE;
					case 'red':
						colorVal = FlxColor.RED;
					case 'yellow':
						colorVal = FlxColor.YELLOW;
					case 'brown':
						colorVal = FlxColor.BROWN;				
					default:
						colorVal = FlxColor.WHITE;
						if(Math.isNaN(colorVal)) colorVal = FlxColor.WHITE;
				}
				var timeVal:Float = Std.parseFloat(value2);
				
				if(ClientPrefs.flashing)
					FlxG.camera.flash(colorVal, timeVal, null, true);	
			case 'Hud Fade':
				var hudVal:Float = Std.parseFloat(value1);
				var timeVal:Float = Std.parseFloat(value2);
				if(Math.isNaN(hudVal)) hudVal = 1;
				FlxTween.tween(camHUD, {alpha: hudVal}, timeVal);
			case 'Notes Fade':
				var notesVal:Float = Std.parseFloat(value1);
				var timeVal:Float = Std.parseFloat(value2);
				if(Math.isNaN(notesVal)) notesVal = 1;
				FlxTween.tween(camNotes, {alpha: notesVal}, timeVal);
			case 'Alternate Attack':
				alternateAttack();
			case 'Captive Popup':
				popupWindow();
			case 'Gaster Popup':
